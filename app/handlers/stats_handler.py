...
from app.keyboards.statistics_keyboard import get_deep_analytics_kb, get_time_spent_criteria_kb, get_portrait_criteria_kb, get_interval_criteria_kb
...
async def ask_traffic_interval_handler(callback: CallbackQuery):
    await callback.message.edit_text(
        text="📈 <b>Оберіть інтервал для групування піків трафіку (Вхід):</b>",
        parse_mode="HTML",
        reply_markup=get_interval_criteria_kb("traffic")
    )
    await callback.answer()


@stats_router.callback_query(F.data.startswith("stat_traffic_"))
async def show_traffic_report_handler(callback: CallbackQuery, session: AsyncSession):
    interval = int(callback.data.split("_")[2])

    answer = await generate_traffic_peaks_report(session, interval_minutes=interval)
...
async def ask_outflow_interval_handler(callback: CallbackQuery):
    await callback.message.edit_text(
        text="🏃‍♂️ <b>Оберіть інтервал для аналізу динаміки відтоку (Вихід):</b>",
        parse_mode="HTML",
        reply_markup=get_interval_criteria_kb("outflow")
    )
    await callback.answer()


@stats_router.callback_query(F.data.startswith("stat_outflow_"))
async def show_outflow_report_handler(callback: CallbackQuery, session: AsyncSession):
    interval = int(callback.data.split("_")[2])

    answer = await generate_outflow_dynamics_report(session, interval_minutes=interval)
